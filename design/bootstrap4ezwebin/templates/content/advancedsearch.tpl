...
<form action={"/content/advancedsearch/"|ezurl} method="get">
    <div class="page-header">
        <h1>{'Advanced search'|i18n( 'design/ezwebin/content/advancedsearch' )}</h1>
    </div>

    <div class="clearfix">
        <label>{'Search all the words'|i18n( 'design/ezwebin/content/advancedsearch' )}</label>
        <div class="input">
            <input class="box" type="text" size="40" name="SearchText" value="{$full_search_text|wash}" />
        </div>
    </div>
    <div class="clearfix">
        <label>{'Search the exact phrase'|i18n( 'design/ezwebin/content/advancedsearch' )}</label>
        <div class="input">
    ...
    <div class="clearfix">
        <label>{'Published'|i18n( 'design/ezwebin/content/advancedsearch' )}</label>
        <div class="input">
    ...
    {if $use_template_search}
    <div class="clearfix">
        <label>{'Display per page'|i18n('design/ezwebin/content/advancedsearch')}</label>
        <div class="input">
    ...
    <div class="alert-message block-message warning">
        <p><strong>{'No results were found when searching for "%1"'|i18n( 'design/ezwebin/content/advancedsearch', , array( $search_text|wash ) )}</strong></p>
    </div>
      {/case}
      {case}
    <div class="alert-message block-message info">
        <p>{'Search for "%1" returned %2 matches'|i18n( 'design/ezwebin/content/advancedsearch',,array( $search_text|wash, $search_count ) )}</p>
    </div>
      {/case}
    {/switch}
    ...
    {foreach $search_result as $element}
       {node_view_gui view=search content_node=$element}
    {/foreach}
    {/if}
