{ezcss_require( 'bootstrap_comment.css' )}
{if $attribute.content.show_comments}
    ...
    {if $can_read}
        ...
        {if $comments|count|gt( 0 )}
            <div id="ezcom-comment-list" class="row ezcom-view-list">
            <div class="span12">
                {if $total_count|gt(0)}
                <div class="ezcom-view-all">
                    ...
                    {else}
                        {if and( is_set( $hide_comments ), $hide_comments )}
                            {def $total_count_string = concat( '<a href=', $contentobject.main_node.url_alias|ezurl, '>', $total_count, '</a>' )}
                        {else}
                            {def $total_count_string = $total_count}
                        {/if}
                          {'Total %total_count comments'|i18n( 'ezcomments/comment/view', , hash( '%total_count', $total_count_string ) )}
                    {/if}
                    </p>
                </div>
                {/if}
